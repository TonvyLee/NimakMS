...
function downloadFile() {
	dorado.MessageBox.confirm("即将开始下载电极握杆三维数模！确认继续？", function() {
		var shankDrawingno = view.id("datagridShank").getCurrentItem().get("shankDrawingno");
		var shankDrawingnoMap = new dorado.util.Map();
		shankDrawingnoMap.put("shankDrawingno", shankDrawingno);
		view.id("downloadDrawing").set("parameter", shankDrawingnoMap).execute();
	});
}
...
!function(self, autoformCondition, dsHolder) {
	...
	var newItemShankMaterial = autoformCondition.get("entity").get("shankMaterial");
	...
	/*// 自动填充下拉框==shankTaper===============================================================
	var newItemShankTaper = autoformCondition.get("entity").get("shankTaper");
	var itemsShankTaper = new Array();
	var objectForce= view.id("listDropDownShankTaper").get("items");
	if(objectForce != null){
	...
	var objectCapAngel= view.id("listDropDownShankType").get("items");
	...
	var newItemShankDesigner = autoformCondition.get("entity").get("shankDesigner");
...
!function(self, updateactionSave) {
	...
	if (returnValue.flag == "S_OK") {
		...
		if(returnValue.flag.indexOf("编码") != -1) {
...
!function(self,arg,ajaxActionJudgeStringIsSatisfied) {
	ajaxActionJudgeStringIsSatisfied.set("parameter",view.id("autoformAdd").get("entity").toJSON()).execute();
};
...
!function(dialog2DDrawing, image) {
	...
	var msg = "${servletContext.getAttribute('configprop').get('TURRETSHANK2D_PATH')}";
	image.set("image", msg + view.id("datagridShank").getCurrentItem().get("shankDrawingno")
			+ ".jpg");
	dialog2DDrawing.show();
};
...
!function(self, downloadDrawing, datagridShank) {
	dorado.MessageBox.confirm("确认下载三维数模？", function() {
		var shankDrawingno = datagridShank.getCurrentItem().get("shankDrawingno");
		var shankDrawingnoMap = new dorado.util.Map();
		shankDrawingnoMap.put("shankDrawingno", shankDrawingno);
		view.id("downloadDrawing").set("parameter", shankDrawingnoMap).execute();
	});
...
!function(arg) {
	...
	arg.dom.innerHTML = "<a href='javascript:view.downloadFile()'>"+shankDrawingno+"</a>";
	// 另外一种实现方式
...
!function(self,arg) {
	//alert(1);
	var entityList =   view.id("dsHolder").getData();
	var entityLast = entityList.getLast();
	//alert(entityFirst.get("shankDrawingno"));
